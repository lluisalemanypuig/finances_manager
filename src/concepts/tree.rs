...
impl Tree {
	...
	*/
	pub fn get_subtree_mut(&mut self, branch: &[String]) -> &mut Tree {
		...
		self.get_child_mut(&branch[0])
			.unwrap()
			.get_subtree_mut(&branch[1..])
	}

	pub fn get_child(&self, s: &String) -> Option<&Tree> {
		let pos = self.m_children.binary_search_by(|e| e.key.cmp(s));
		match pos {
			Ok(idx) => match &self.m_children[idx].tree {
				Some(t) => Some(t),
				None => None,
			},
			Err(_) => None,
	...
	pub fn get_child_mut(&mut self, s: &String) -> Option<&mut Tree> {
		let pos = self.m_children.binary_search_by(|e| e.key.cmp(s));
		match pos {
			Ok(idx) => match &mut self.m_children[idx].tree {
				Some(t) => Some(t),
				None => None,
			},
			Err(_) => None,
	...
	pub fn remove_child(&mut self, key: &String) -> &mut Tree {
		let pos = self.m_children.binary_search_by(|e| e.key.cmp(key));
		match pos {
			Ok(idx) => {
	...
	pub fn rename_key(&mut self, old_key: &String, new_key: String) -> &mut Tree {
		let pos = self.m_children.binary_search_by(|e| e.key.cmp(old_key));
	...
	pub fn insert_key(&mut self, key: String, tree: Option<Tree>) -> &mut Tree {
		...
		match pos {
			...
			Err(idx) => {
				self.m_children.insert(idx, KeyTree { key, tree });
			},
		}
		self
	}
