...
impl AllActivities {
	...
	pub fn iter_activities(&self) -> std::slice::Iter<'_, YearlyActivities> {
	...
	pub fn get_expense_concepts(&self) -> &ConceptTypes {
		&self.m_expense_types
	}
	...
	pub fn get_income_concepts(&self) -> &ConceptTypes {
		&self.m_income_types
	}
	...
	pub fn has_year(&self, y: &u32) -> bool {
		self.m_activities
			.binary_search_by(|e| e.get_year().cmp(&y))
			.is_ok()
	}
	...
	pub fn add_year(&mut self, y: u32) -> &mut YearlyActivities {
		...
		match res {
			...
			Err(pos) => {
				// year does not exist
				self.m_activities.insert(pos, YearlyActivities::new_year(y));
	...
	pub fn push_year(&mut self, y: YearlyActivities) {
		let res = self
			.m_activities
			.binary_search_by(|e| e.get_year().cmp(&y.get_year()));
		match res {
			Ok(_) => {},
			Err(pos) => {
	...
	pub fn merge(&mut self, year_acts: YearlyActivities) {
		if !self.has_year(&year_acts.get_year()) {
			...
			self.get_year_mut(&year_acts.get_year())
				.unwrap()
				.merge(year_acts);
		}
	}
